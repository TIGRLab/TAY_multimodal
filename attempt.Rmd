...
```
```{r}
#want to make this into a read_delim file like earlier
desc_aparcaseg_model_noddi_results <- files_list %>% map_dfr(~ read_delim(.x, delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>% mutate(source_file=.x))

#desc_aparcaseg_model_noddi_results <- map_dfr(files_list, function(x) {read_delim (x, delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>% mutate(source_file=x)})
```
